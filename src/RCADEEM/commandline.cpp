...
static s_command_parameter _command_parameters[] = {
	...
	{	"Input file containing the sequences",
		"-fasta", _TYPE_STRING, true, true, 0, 0.0, "None" },
	{	"Enrichment fold expected for each ZF (negative means PWMs should not be normalized)",
		"-enf", _TYPE_DOUBLE, true, true, 0, 50.0, "None" },
	{	"Optimization mode (1: optimized AUC; 2: optimize AUC x Similarity; 3: until convergence)",
		"-mode", _TYPE_INT, true, true, 1, 0.0, "None" },
	{	"The experiment name",
		"-exp", _TYPE_STRING, true, true, 0, 0.0, "" },
	{	"Output file",
		"-out"  , _TYPE_STRING, true, false, 0, 0.0, "None" } };

int _num_parameters = 8;
...
const char *__fasta_file = _command_parameters[ 3 ].v_string;
const double *__enrichment_fold = & _command_parameters[ 4 ].v_double;
const int *__mode = & _command_parameters[ 5 ].v_int;
const char *__experiment = _command_parameters[ 6 ].v_string;
const char *__output_file = _command_parameters[ 7 ].v_string;
