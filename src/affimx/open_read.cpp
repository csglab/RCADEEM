...
bool open_motifs(
	...
	int *num_motifs,
	int *is_PWM )
// opens a PWM file and reads its content
// returns true if successful, false otherwise
//
// motif_file: the path and name of the PWM file to be read
// motifs: the motif list whose sequence information will be read
// num_motifs: the motif list size
{
	// open the FASTA file
	ifstream ifs;
	_CALL(
		_open_input( ifs, motif_file, ".txt" )
		);
	...
	// read the motif file
	cout << "Reading PWMs..." << endl;
	_CALL(
		read_motifs( ifs, motifs, num_motifs, is_PWM, this_EOL )
		);
		
	cout << *num_motifs << " motifs were read." << endl;
	if( *is_PWM )
		cout << "The input was interpreted to present PWMs." << endl << endl;
	else
		cout << "The input was interpreted to present PFMs." << endl << endl;
